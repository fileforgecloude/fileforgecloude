...
import { useCreateFile, useDeleteFile, useFiles } from "@/react-query/files/file-actions";
  ...
  const driveFiles: DriveFile[] = useMemo(() => {
    ...
    if (filterType) {
      result = result.filter((file) => {
        ...
        if (filterType === "code")
          return file.type.includes("typescript") || file.type.includes("javascript") || file.type.includes("json");
        return true;
      });
    }
  ...
  return (
    <div className=''>
      ...
      <ConfirmationDialog
        ...
        description={
          <>
            Are you sure you want to delete <span className='font-bold text-slate-800 dark:text-slate-200'>"{fileToDelete?.name}"</span>?
